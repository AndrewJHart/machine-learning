...
# Create another layer for funsies, detecting 128 features in a 5x5 patch.
# Note that this results in a slightly lower accuracy overall.
# W_conv3 = weight_variable([5, 5, 64, 128])
# b_conv3 = bias_variable([128])
# h_conv3 = tf.nn.relu(conv2d(h_pool2, W_conv3) + b_conv3)
# h_pool3 = max_pool_2x2(h_conv3)

# When adding another layour, we need to adjust our fully connected
...
# Identify our features with a fully connected layer
# before we pass them through our readout layer.
W_fc1 = weight_variable([7 * 7 * 64, 1024])
b_fc1 = bias_variable([1024])
h_pool2_flat = tf.reshape(h_pool2, [-1, 7 * 7 * 64])
h_fc1 = tf.nn.relu(tf.matmul(h_pool2_flat, W_fc1) + b_fc1)

# Setup dropout to handle overfitting.
keep_prob = tf.placeholder(tf.float32)
h_fc1_drop = tf.nn.dropout(h_fc1, keep_prob)

# Finally do the actual neural network readout with our readout layer.
W_fc2 = weight_variable([1024, 10])
b_fc2 = bias_variable([10])
prediction = tf.matmul(h_fc1_drop, W_fc2) + b_fc2

# Now we train the model with an Adam Optimizer instead and a smaller learning rate value.
